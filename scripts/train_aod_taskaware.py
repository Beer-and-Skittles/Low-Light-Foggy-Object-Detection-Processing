...
def main(cfg, log_path, ckpt_path=None):
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        # ---- Save checkpoint ----
        if (epoch) % 50 == 0:
            ckpt_path = out_dir / f"aod_epoch{epoch}.pt"
            torch.save({
                "epoch": epoch,
                "aod": aod.state_dict(),
                "opt": opt.state_dict(),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--cfg", default="configs/aod_train.yaml")
    ap.add_argument("--ckpt_path", default=None,
                    help="Path to AOD checkpoint (.pt) to resume from")
    ap.add_argument("--log_path", default="outputs/aod_loss.txt")
    args = ap.parse_args()
...
def main(cfg, log_path, ckpt_path=None):
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        if avg_total < best_loss:
            best_loss = avg_total
            best_ckpt_path = out_dir / "aod_best.pt"
            torch.save({
                "epoch": epoch,
                "aod": aod.state_dict(),
                "opt": opt.state_dict(),
